...
@export var room_border_px = Vector2(16, 16)
...
func setup_doorway_collision(rs):
	...
	if room_size:
		player_detector_collision.shape.size = room_size - (2 * room_border_px)
	...
	if up:
		...
		up_collision.position = Vector2(room_size.x / 2, 8)
	...
	if down:
		...
		down_collision.position = Vector2(room_size.x / 2, room_size.y - 8)
